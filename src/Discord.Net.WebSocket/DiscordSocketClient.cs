...
#pragma warning restore IDISP004
            ...
            : base(config, client)
        {
            ...
            _connection.Connected += () => _ = TimedInvokeAsync(_connectedEvent, nameof(Connected));
            _connection.Disconnected += (ex, recon) => _ = TimedInvokeAsync(_disconnectedEvent, nameof(Disconnected), ex);
            ...
            ApiClient.SentGatewayMessage += opCode => _gatewayLogger.DebugAsync($"Sent {opCode}");
            ApiClient.ReceivedGatewayEvent += ProcessMessageAsync;

            LeftGuild += g => _gatewayLogger.InfoAsync($"Left {g.Name}");
            JoinedGuild += g => _gatewayLogger.InfoAsync($"Joined {g.Name}");
            GuildAvailable += g => _gatewayLogger.VerboseAsync($"Connected to {g.Name}");
            GuildUnavailable += g => _gatewayLogger.VerboseAsync($"Disconnected from {g.Name}");
            LatencyUpdated += (old, val) => _gatewayLogger.DebugAsync($"Latency = {val} ms");
        ...
        private async Task OnConnectingAsync()
        {
            ...
            {
                await _shardedClient.AcquireIdentifyLockAsync(ShardId, _connection.CancelToken);
                locked = true;
            }
            try
            {
                await _gatewayLogger.DebugAsync("Connecting ApiClient");
                await ApiClient.ConnectAsync();

                if (_sessionId != null)
                {
                    await _gatewayLogger.DebugAsync("Resuming");
                    await ApiClient.SendResumeAsync(_sessionId, _lastSeq);
                }
                else
                {
                    await _gatewayLogger.DebugAsync("Identifying");
                    await ApiClient.SendIdentifyAsync(shardID: ShardId, totalShards: TotalShards, gatewayIntents: _gatewayIntents, presence: BuildCurrentStatus());
                }
            }
            finally
            {
            ...
            if (LogGatewayIntentWarnings)
                await LogGatewayIntentsWarning();
        }
        private async Task OnDisconnectingAsync(Exception ex)
        {
            await _gatewayLogger.DebugAsync("Disconnecting ApiClient");
            await ApiClient.DisconnectAsync(ex);

            //Wait for tasks to complete
            await _gatewayLogger.DebugAsync("Waiting for heartbeater");
            ...
            await _gatewayLogger.DebugAsync("Waiting for guild downloader");
            var guildDownloadTask = _guildDownloadTask;
            if (guildDownloadTask != null)
                await guildDownloadTask;
            _guildDownloadTask = null;

            //Clear large guild queue
            await _gatewayLogger.DebugAsync("Clearing large guild queue");
            while (_largeGuilds.TryDequeue(out _))
            { }

            //Raise virtual GUILD_UNAVAILABLEs
            await _gatewayLogger.DebugAsync("Raising virtual GuildUnavailables");
            foreach (var guild in State.Guilds)
            {
                if (guild.IsAvailable)
                    await GuildUnavailableAsync(guild);
            }
        }

        /// <inheritdoc />
        public override async Task<RestApplication> GetApplicationInfoAsync(RequestOptions options = null)
            => _applicationInfo ??= await ClientHelper.GetApplicationInfoAsync(this, options ?? RequestOptions.Default);
        ...
        public async ValueTask<IChannel> GetChannelAsync(ulong id, RequestOptions options = null)
            => GetChannel(id) ?? (IChannel)await ClientHelper.GetChannelAsync(this, id, options);
        /// <summary>
        ...
        public async ValueTask<IUser> GetUserAsync(ulong id, RequestOptions options = null)
            => await ((IDiscordClient)this).GetUserAsync(id, CacheMode.AllowDownload, options);
        /// <summary>
        ...
        {
            var model = await InteractionHelper.CreateGlobalCommandAsync(this, properties, options);
        ...
        public override async ValueTask<IReadOnlyCollection<RestVoiceRegion>> GetVoiceRegionsAsync(RequestOptions options = null)
        {
            if (_parentClient == null)
            {
                if (_voiceRegions == null)
                {
                    ...
                    var voiceRegions = await ApiClient.GetVoiceRegionsAsync(options);
            ...
            return await _parentClient.GetVoiceRegionsAsync();
        }
        ...
        public override async ValueTask<RestVoiceRegion> GetVoiceRegionAsync(string id, RequestOptions options = null)
        {
            ...
            {
                if (_voiceRegions == null)
                    await GetVoiceRegionsAsync();
                if (_voiceRegions.TryGetValue(id, out RestVoiceRegion region))
                    return region;
                return null;
            }
            return await _parentClient.GetVoiceRegionAsync(id, options);
        }
        ...
        private async Task ProcessUserDownloadsAsync(IEnumerable<SocketGuild> guilds)
        {
            ...
            for (int i = 0, k = 0; i < batchCount; i++)
            {
                ...
                await ApiClient.SendRequestMembersAsync(batchIds);

                if (isLast && batchCount > 1)
                    await Task.WhenAll(batchTasks.Take(count));
                else
        ...
        private async Task LogGatewayIntentsWarning()
        {
            ...
            {
                await _gatewayLogger.WarningAsync("You're using the GuildPresences intent without listening to the PresenceUpdate event, consider removing the intent from your config.");
            }
            ...
            if (!_gatewayIntents.HasFlag(GatewayIntents.GuildPresences) &&
               ((_shardedClient is null && _presenceUpdated.HasSubscribers) ||
                (_shardedClient is not null && _shardedClient._presenceUpdated.HasSubscribers)))
            {
                await _gatewayLogger.WarningAsync("You're using the PresenceUpdate event without specifying the GuildPresences intent. Discord wont send this event to your client without the intent set in your config.");
            }
            ...
            {
                await _gatewayLogger.WarningAsync("You're using the GuildScheduledEvents gateway intent without listening to any events related to that intent, consider removing the intent from your config.");
            }
            ...
            if (!_gatewayIntents.HasFlag(GatewayIntents.GuildScheduledEvents) &&
               ((_shardedClient is null && hasGuildScheduledEventsSubscribers) ||
                (_shardedClient is not null && shardedClientHasGuildScheduledEventsSubscribers)))
            {
                await _gatewayLogger.WarningAsync("You're using events related to the GuildScheduledEvents gateway intent without specifying the intent. Discord wont send this event to your client without the intent set in your config.");
            }
            ...
            {
                await _gatewayLogger.WarningAsync("You're using the GuildInvites gateway intent without listening to any events related to that intent, consider removing the intent from your config.");
            }
            ...
            if (!_gatewayIntents.HasFlag(GatewayIntents.GuildInvites) &&
                ...
                (_shardedClient is not null && shardedClientHasInviteEventSubscribers)))
            {
                await _gatewayLogger.WarningAsync("You're using events related to the GuildInvites gateway intent without specifying the intent. Discord wont send this event to your client without the intent set in your config.");
            }
        }

        #region ProcessMessageAsync

        private Task HandleInvokeException(Task task, string name = null)
        {
            if(task.IsFaulted)
                return _gatewayLogger.WarningAsync($"A {(name != null ? $" ({name})" : "")} handler has thrown an unhandled exception.", task.Exception);
            ...
            try
            {
                switch (opCode)
                {
                    case GatewayOpCode.Hello:
                        {
                            await _gatewayLogger.DebugAsync("Received Hello");
                    ...
                    case GatewayOpCode.Heartbeat:
                        {
                            await _gatewayLogger.DebugAsync("Received Heartbeat");

                            await ApiClient.SendHeartbeatAsync(_lastSeq);
                        }
                        break;
                    case GatewayOpCode.HeartbeatAck:
                        {
                            await _gatewayLogger.DebugAsync("Received HeartbeatAck");
                            ...
                            if (_heartbeatTimes.TryDequeue(out long time))
                            {
                                ...
                                _ = TimedInvokeAsync(_latencyUpdatedEvent, nameof(LatencyUpdated), before, latency).ContinueWith(t => HandleInvokeException(t, nameof(LatencyUpdated)));
                            }
                        }
                        break;
                    case GatewayOpCode.InvalidSession:
                        {
                            await _gatewayLogger.DebugAsync("Received InvalidSession");
                    ...
                    case GatewayOpCode.Reconnect:
                        {
                            await _gatewayLogger.DebugAsync("Received Reconnect");
                    ...
                    case GatewayOpCode.Dispatch:
                        switch (type)
                        {
                            ...
                            case "READY":
                                {
                                    try
                                    {
                                        await _gatewayLogger.DebugAsync("Received Dispatch (READY)");
                                        ...
                                        for (int i = 0; i < data.Guilds.Length; i++)
                                        {
                                            ...
                                            if (!guild.IsAvailable)
                                                ...
                                                await GuildAvailableAsync(guild);
                                        }
                                    ...
                                    _guildDownloadTask = WaitForGuildsAsync(_connection.CancelToken, _gatewayLogger)
                                        .ContinueWith(async x =>
                                        {
                                            ...
                                            _ = TimedInvokeAsync(_readyEvent, nameof(Ready)).ContinueWith(t => HandleInvokeException(t, nameof(Ready)));
                            ...
                            case "RESUMED":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (RESUMED)");
                                    ...
                                    await _gatewayLogger.InfoAsync("Resumed previous session");
                                }
                                break;
                            #endregion
                            ...
                            case "GUILD_CREATE":
                                {
                                    ...
                                    if (data.Unavailable == false)
                                    {
                                        ...
                                        if (guild != null)
                                        {
                                            ...
                                            if (guild.DownloadedMemberCount >= guild.MemberCount && !guild.DownloaderPromise.IsCompleted)
                                            {
                                                ...
                                                _ = TimedInvokeAsync(_guildMembersDownloadedEvent, nameof(GuildMembersDownloaded), guild).ContinueWith(t => HandleInvokeException(t, nameof(GuildMembersDownloaded)));
                                            }
                                        }
                                        else
                                        {
                                            await UnknownGuildAsync(type, data.Id);
                                            return;
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        if (guild != null)
                                        {
                                            _ = TimedInvokeAsync(_joinedGuildEvent, nameof(JoinedGuild), guild).ContinueWith(t => HandleInvokeException(t, nameof(JoinedGuild)));
                                            await GuildAvailableAsync(guild);
                                        }
                                        else
                                        {
                            ...
                            case "GUILD_UPDATE":
                                {
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        guild.Update(State, data);
                                        _ = TimedInvokeAsync(_guildUpdatedEvent, nameof(GuildUpdated), before, guild).ContinueWith(t => HandleInvokeException(t, nameof(GuildUpdated)));
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.Id);
                            ...
                            case "GUILD_EMOJIS_UPDATE":
                                {
                                    ...
                                    {
                                        await UnknownGuildAsync(type, data.GuildId);
                                        return;
                                    }
                                }
                                break;
                            case "GUILD_SYNC":
                                {
                                    await _gatewayLogger.DebugAsync("Ignored Dispatch (GUILD_SYNC)");
                                    /*await _gatewayLogger.DebugAsync("Received Dispatch (GUILD_SYNC)"); //TODO remove? userbot related
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        _lastGuildAvailableTime = Environment.TickCount;
                                        await GuildAvailableAsync(guild);
                                        _ = TimedInvokeAsync(_guildUpdatedEvent, nameof(GuildUpdated), before, guild);
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.Id);
                            ...
                            case "GUILD_DELETE":
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            await GuildUnavailableAsync(guild);
                                            _ = TimedInvokeAsync(_leftGuildEvent, nameof(LeftGuild), guild).ContinueWith(t => HandleInvokeException(t, nameof(LeftGuild)));
                            ...
                            case "GUILD_STICKERS_UPDATE":
                                {
                                    await _gatewayLogger.DebugAsync($"Received Dispatch (GUILD_STICKERS_UPDATE)");
                                    ...
                                    foreach (var model in newStickers)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_guildStickerCreated, nameof(GuildStickerCreated), entity).ContinueWith(t => HandleInvokeException(t, nameof(GuildStickerCreated)));
                                    }
                                    foreach (var sticker in deletedStickers)
                                    {
                                        var entity = guild.RemoveSticker(sticker.Id);
                                        _ = TimedInvokeAsync(_guildStickerDeleted, nameof(GuildStickerDeleted), entity).ContinueWith(t => HandleInvokeException(t, nameof(GuildStickerDeleted)));
                                    }
                                    foreach (var entityModelPair in updatedStickers)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_guildStickerUpdated, nameof(GuildStickerUpdated), before, entityModelPair.Entity).ContinueWith(t => HandleInvokeException(t, nameof(GuildStickerUpdated)));
                                    }
                                }
                                break;
                            #endregion
                            ...
                            case "CHANNEL_CREATE":
                                {
                                    ...
                                    if (data.GuildId.IsSpecified)
                                    {
                                        ...
                                        if (guild != null)
                                        {
                                            ...
                                            if (!guild.IsSynced)
                                            {
                                                await UnsyncedGuildAsync(type, guild.Id);
                                                return;
                                            }
                                        }
                                        else
                                    ...
                                    if (channel != null)
                                        _ = TimedInvokeAsync(_channelCreatedEvent, nameof(ChannelCreated), channel).ContinueWith(t => HandleInvokeException(t, nameof(ChannelCreated)));    
                                }
                                break;
                            case "CHANNEL_UPDATE":
                                {
                                    ...
                                    if (channel != null)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_channelUpdatedEvent, nameof(ChannelUpdated), before, channel).ContinueWith(t => HandleInvokeException(t, nameof(ChannelUpdated)));
                                    }
                                    else
                                    {
                            ...
                            case "CHANNEL_DELETE":
                                {
                                    ...
                                    if (channel != null)
                                        _ = TimedInvokeAsync(_channelDestroyedEvent, nameof(ChannelDestroyed), channel).ContinueWith(t => HandleInvokeException(t, nameof(ChannelDestroyed)));
                                    else
                                    {
                            ...
                            case "GUILD_MEMBER_ADD":
                                {
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_userJoinedEvent, nameof(UserJoined), user).ContinueWith(t => HandleInvokeException(t, nameof(UserJoined)));
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.GuildId);
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        if (user != null)
                                        {
                                            ...
                                            if (user.GlobalUser.Update(State, data.User))
                                            {
                                                ...
                                                _ = TimedInvokeAsync(_userUpdatedEvent, nameof(UserUpdated), before.GlobalUser, user).ContinueWith(t => HandleInvokeException(t, nameof(UserUpdated)));
                                            }

                                            user.Update(State, data);

                                            var cacheableBefore = new Cacheable<SocketGuildUser, ulong>(before, user.Id, true, () => Task.FromResult<SocketGuildUser>(null));
                                            _ = TimedInvokeAsync(_guildMemberUpdatedEvent, nameof(GuildMemberUpdated), cacheableBefore, user).ContinueWith(t => HandleInvokeException(t, nameof(GuildMemberUpdated)));
                                        }
                                        else
                                        {
                            ...
                            case "GUILD_MEMBER_REMOVE":
                                {
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_userLeftEvent, nameof(UserLeft), guild, user).ContinueWith(t => HandleInvokeException(t, nameof(UserLeft)));
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.GuildId);
                            ...
                            case "GUILD_JOIN_REQUEST_DELETE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (GUILD_JOIN_REQUEST_DELETE)");
                                    ...
                                    _ = TimedInvokeAsync(_guildJoinRequestDeletedEvent, nameof(GuildJoinRequestDeleted), cacheableUser, guild).ContinueWith(t => HandleInvokeException(t, nameof(GuildJoinRequestDeleted)));
                                }
                                break;
                            #endregion
                            ...
                            case "CHANNEL_RECIPIENT_ADD":
                                {
                                    ...
                                    if (State.GetChannel(data.ChannelId) is SocketGroupChannel channel)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_recipientAddedEvent, nameof(RecipientAdded), user).ContinueWith(t => HandleInvokeException(t, nameof(RecipientAdded)));
                                    }
                                    else
                                    {
                                        await UnknownChannelAsync(type, data.ChannelId);
                                        return;
                                    }
                                }
                                break;
                            case "CHANNEL_RECIPIENT_REMOVE":
                                {
                                    ...
                                    if (State.GetChannel(data.ChannelId) is SocketGroupChannel channel)
                                    {
                                        ...
                                        if (user != null)
                                            _ = TimedInvokeAsync(_recipientRemovedEvent, nameof(RecipientRemoved), user).ContinueWith(t => HandleInvokeException(t, nameof(RecipientRemoved)));
                                        else
                                        {
                            ...
                            case "GUILD_ROLE_CREATE":
                                {
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_roleCreatedEvent, nameof(RoleCreated), role).ContinueWith(t => HandleInvokeException(t, nameof(RoleCreated)));
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.GuildId);
                            ...
                            case "GUILD_ROLE_UPDATE":
                                {
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        if (role != null)
                                        {
                                            ...
                                            _ = TimedInvokeAsync(_roleUpdatedEvent, nameof(RoleUpdated), before, role).ContinueWith(t => HandleInvokeException(t, nameof(RoleUpdated)));
                                        }
                                        else
                                        {
                                            await UnknownRoleAsync(type, data.Role.Id, guild.Id);
                                            return;
                                        }
                                    }
                                    else
                            ...
                            case "GUILD_ROLE_DELETE":
                                {
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        if (role != null)
                                        {
                                            ...
                                            _ = TimedInvokeAsync(_roleDeletedEvent, nameof(RoleDeleted), role).ContinueWith(t => HandleInvokeException(t, nameof(RoleDeleted)));
                                        }
                                        else
                                        {
                                            await UnknownRoleAsync(type, data.RoleId, guild.Id);
                                            return;
                                        }
                                    }
                                    else
                            ...
                            case "GUILD_BAN_ADD":
                                {
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_userBannedEvent, nameof(UserBanned), user, guild).ContinueWith(t => HandleInvokeException(t, nameof(UserBanned)));
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.GuildId);
                            ...
                            case "GUILD_BAN_REMOVE":
                                {
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        if (user == null)
                                            user = SocketUnknownUser.Create(this, State, data.User);
                                        _ = TimedInvokeAsync(_userUnbannedEvent, nameof(UserUnbanned), user, guild).ContinueWith(t => HandleInvokeException(t, nameof(UserUnbanned)));
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.GuildId);
                            ...
                            case "MESSAGE_CREATE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_messageReceivedEvent, nameof(MessageReceived), msg).ContinueWith(t => HandleInvokeException(t, nameof(MessageReceived)));
                                }
                                break;
                            case "MESSAGE_UPDATE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_messageUpdatedEvent, nameof(MessageUpdated), cacheableBefore, after, channel).ContinueWith(t => HandleInvokeException(t, nameof(MessageUpdated)));
                                }
                                break;
                            case "MESSAGE_DELETE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_messageDeletedEvent, nameof(MessageDeleted), cacheableMsg, cacheableChannel).ContinueWith(t => HandleInvokeException(t, nameof(MessageDeleted)));
                                }
                                break;
                            case "MESSAGE_REACTION_ADD":
                                {
                                    ...
                                    IUser user = null;
                                    if (channel != null)
                                        user = await channel.GetUserAsync(data.UserId, CacheMode.CacheOnly);
                                    ...
                                    {
                                        var channelObj = await cacheableChannel.GetOrDownloadAsync();
                                    ...
                                    _ = TimedInvokeAsync(_reactionAddedEvent, nameof(ReactionAdded), cacheableMsg, cacheableChannel, reaction).ContinueWith(t => HandleInvokeException(t, nameof(ReactionAdded)));
                                }
                                break;
                            case "MESSAGE_REACTION_REMOVE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_reactionRemovedEvent, nameof(ReactionRemoved), cacheableMsg, cacheableChannel, reaction).ContinueWith(t => HandleInvokeException(t, nameof(ReactionRemoved)));
                                }
                                break;
                            case "MESSAGE_REACTION_REMOVE_ALL":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_reactionsClearedEvent, nameof(ReactionsCleared), cacheableMsg, cacheableChannel).ContinueWith(t => HandleInvokeException(t, nameof(ReactionsCleared)));
                                }
                                break;
                            case "MESSAGE_REACTION_REMOVE_EMOJI":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (MESSAGE_REACTION_REMOVE_EMOJI)");
                                    ...
                                    _ = TimedInvokeAsync(_reactionsRemovedForEmoteEvent, nameof(ReactionsRemovedForEmote), cacheableMsg, cacheableChannel, emote).ContinueWith(t => HandleInvokeException(t, nameof(ReactionsRemovedForEmote)));
                                }
                                break;
                            case "MESSAGE_DELETE_BULK":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_messagesBulkDeletedEvent, nameof(MessagesBulkDeleted), cacheableList, cacheableChannel).ContinueWith(t => HandleInvokeException(t, nameof(MessagesBulkDeleted)));
                                }
                                break;
                            #endregion
                            ...
                            case "PRESENCE_UPDATE":
                                {
                                    ...
                                    if (data.GuildId.IsSpecified)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                //Global data was updated, trigger UserUpdated
                                                _ = TimedInvokeAsync(_userUpdatedEvent, nameof(UserUpdated), globalBefore, user).ContinueWith(t => HandleInvokeException(t, nameof(UserUpdated)));
                                            }
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        if (user == null)
                                        {
                                            await UnknownGlobalUserAsync(type, data.User.Id);
                                            return;
                                        }
                                    }
                                    ...
                                    user.Update(State, data.User);
                                    user.Update(data);
                                    _ = TimedInvokeAsync(_presenceUpdated, nameof(PresenceUpdated), user, before, user.Presence).ContinueWith(t => HandleInvokeException(t, nameof(PresenceUpdated)));
                                }
                                break;
                            case "TYPING_START":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_userIsTypingEvent, nameof(UserIsTyping), cacheableUser, cacheableChannel).ContinueWith(t => HandleInvokeException(t, nameof(UserIsTyping)));
                                }
                                break;
                            #endregion
                            ...
                            case "INTEGRATION_CREATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (INTEGRATION_CREATE)");
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_integrationCreated, nameof(IntegrationCreated), RestIntegration.Create(this, guild, data)).ContinueWith(t => HandleInvokeException(t, nameof(IntegrationCreated)));
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.GuildId.Value);
                            ...
                            case "INTEGRATION_UPDATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (INTEGRATION_UPDATE)");
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_integrationUpdated, nameof(IntegrationUpdated), RestIntegration.Create(this, guild, data)).ContinueWith(t => HandleInvokeException(t, nameof(IntegrationUpdated)));
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.GuildId.Value);
                            ...
                            case "INTEGRATION_DELETE":
                                {
                                    ...
                                    if (guild != null)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_integrationDeleted, nameof(IntegrationDeleted), guild, data.Id, data.ApplicationID).ContinueWith(t => HandleInvokeException(t, nameof(IntegrationDeleted)));
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.GuildId);
                            ...
                            case "USER_UPDATE":
                                {
                                    ...
                                    if (data.Id == CurrentUser.Id)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_selfUpdatedEvent, nameof(CurrentUserUpdated), before, CurrentUser).ContinueWith(t => HandleInvokeException(t, nameof(CurrentUserUpdated)));
                                    }
                                    else
                                    {
                            ...
                            case "VOICE_STATE_UPDATE":
                                {
                                    ...
                                    if (data.GuildId != null)
                                    {
                                        ...
                                        if (data.ChannelId != null)
                                        {
                                            ...
                                            {
                                                var _ = guild.FinishJoinAudioChannel();
                                            }*/
                                        }
                                        else
                                        {
                                        ...
                                        if (user == null)
                                        {
                                            await UnknownGuildUserAsync(type, data.UserId, guild.Id);
                                            return;
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            await UnknownChannelAsync(type, data.ChannelId.Value);
                                            return;
                                        }
                                        if (data.ChannelId != null)
                                        {
                                    ...
                                    if (user is SocketGuildUser guildUser && data.ChannelId.HasValue)
                                    {
                                        ...
                                        if (stage != null && before.VoiceChannel != null && after.VoiceChannel != null)
                                        {
                                            ...
                                            {
                                                _ = TimedInvokeAsync(_requestToSpeak, nameof(RequestToSpeak), stage, guildUser).ContinueWith(t => HandleInvokeException(t, nameof(RequestToSpeak)));
                                                return;
                                            }
                                            if (before.IsSuppressed && !after.IsSuppressed)
                                            {
                                                _ = TimedInvokeAsync(_speakerAdded, nameof(SpeakerAdded), stage, guildUser).ContinueWith(t => HandleInvokeException(t, nameof(SpeakerAdded)));
                                                return;
                                            }
                                            if (!before.IsSuppressed && after.IsSuppressed)
                                            {
                                                _ = TimedInvokeAsync(_speakerRemoved, nameof(SpeakerRemoved), stage, guildUser).ContinueWith(t => HandleInvokeException(t, nameof(SpeakerRemoved)));
                                            }
                                        }
                                    }

                                    _ = TimedInvokeAsync(_userVoiceStateUpdatedEvent, nameof(UserVoiceStateUpdated), user, before, after).ContinueWith(t => HandleInvokeException(t, nameof(UserVoiceStateUpdated)));
                                }
                                break;
                            case "VOICE_SERVER_UPDATE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_voiceServerUpdatedEvent, nameof(UserVoiceStateUpdated), voiceServer).ContinueWith(t => HandleInvokeException(t, nameof(UserVoiceStateUpdated)));
                                    ...
                                    if (isCached)
                                    {
                                        ...
                                        var _ = guild.FinishConnectAudio(endpoint, data.Token);
                                    }
                                    else
                                    {
                                        await UnknownGuildAsync(type, data.GuildId);
                            ...
                            case "INVITE_CREATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (INVITE_CREATE)");
                                    ...
                                    if (State.GetChannel(data.ChannelId) is SocketGuildChannel channel)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_inviteCreatedEvent, nameof(InviteCreated), invite).ContinueWith(t => HandleInvokeException(t, nameof(InviteCreated)));
                                    }
                                    else
                                    {
                                        await UnknownChannelAsync(type, data.ChannelId);
                            ...
                            case "INVITE_DELETE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (INVITE_DELETE)");
                                    ...
                                    if (State.GetChannel(data.ChannelId) is SocketGuildChannel channel)
                                    {
                                        ...
                                        _ = TimedInvokeAsync(_inviteDeletedEvent, nameof(InviteDeleted), channel, data.Code).ContinueWith(t => HandleInvokeException(t, nameof(InviteDeleted)));
                                    }
                                    else
                                    {
                                        await UnknownChannelAsync(type, data.ChannelId);
                            ...
                            case "INTERACTION_CREATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (INTERACTION_CREATE)");
                                    ...
                                    _ = TimedInvokeAsync(_interactionCreatedEvent, nameof(InteractionCreated), interaction).ContinueWith(t => HandleInvokeException(t, nameof(InteractionCreated)));

                                    switch (interaction)
                                    {
                                        case SocketSlashCommand slashCommand:
                                            _ = TimedInvokeAsync(_slashCommandExecuted, nameof(SlashCommandExecuted), slashCommand).ContinueWith(t => HandleInvokeException(t, nameof(SlashCommandExecuted)));
                                            break;
                                        case SocketMessageComponent messageComponent:
                                            if (messageComponent.Data.Type.IsSelectType())
                                                _ = TimedInvokeAsync(_selectMenuExecuted, nameof(SelectMenuExecuted), messageComponent).ContinueWith(t => HandleInvokeException(t, nameof(SelectMenuExecuted)));
                                            if (messageComponent.Data.Type == ComponentType.Button)
                                                _ = TimedInvokeAsync(_buttonExecuted, nameof(ButtonExecuted), messageComponent).ContinueWith(t => HandleInvokeException(t, nameof(ButtonExecuted)));
                                            break;
                                        case SocketUserCommand userCommand:
                                            _ = TimedInvokeAsync(_userCommandExecuted, nameof(UserCommandExecuted), userCommand).ContinueWith(t => HandleInvokeException(t, nameof(UserCommandExecuted)));
                                            break;
                                        case SocketMessageCommand messageCommand:
                                            _ = TimedInvokeAsync(_messageCommandExecuted, nameof(MessageCommandExecuted), messageCommand).ContinueWith(t => HandleInvokeException(t, nameof(MessageCommandExecuted)));
                                            break;
                                        case SocketAutocompleteInteraction autocomplete:
                                            _ = TimedInvokeAsync(_autocompleteExecuted, nameof(AutocompleteExecuted), autocomplete).ContinueWith(t => HandleInvokeException(t, nameof(AutocompleteExecuted)));
                                            break;
                                        case SocketModal modal:
                                            _ = TimedInvokeAsync(_modalSubmitted, nameof(ModalSubmitted), modal).ContinueWith(t => HandleInvokeException(t, nameof(ModalSubmitted)));
                                            break;
                                    }
                                }
                                break;
                            case "APPLICATION_COMMAND_CREATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (APPLICATION_COMMAND_CREATE)");
                                    ...
                                    _ = TimedInvokeAsync(_applicationCommandCreated, nameof(ApplicationCommandCreated), applicationCommand).ContinueWith(t => HandleInvokeException(t, nameof(ApplicationCommandCreated)));
                                }
                                break;
                            case "APPLICATION_COMMAND_UPDATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (APPLICATION_COMMAND_UPDATE)");
                                    ...
                                    _ = TimedInvokeAsync(_applicationCommandUpdated, nameof(ApplicationCommandUpdated), applicationCommand).ContinueWith(t => HandleInvokeException(t, nameof(ApplicationCommandUpdated)));
                                }
                                break;
                            case "APPLICATION_COMMAND_DELETE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (APPLICATION_COMMAND_DELETE)");
                                    ...
                                    _ = TimedInvokeAsync(_applicationCommandDeleted, nameof(ApplicationCommandDeleted), applicationCommand).ContinueWith(t => HandleInvokeException(t, nameof(ApplicationCommandDeleted)));
                                }
                                break;
                            #endregion
                            ...
                            case "THREAD_CREATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (THREAD_CREATE)");
                                    ...
                                    _ = TimedInvokeAsync(_threadCreated, nameof(ThreadCreated), threadChannel).ContinueWith(t => HandleInvokeException(t, nameof(ThreadCreated)));
                                }

                                break;
                            case "THREAD_UPDATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (THREAD_UPDATE)");
                                    ...
                                    _ = TimedInvokeAsync(_threadUpdated, nameof(ThreadUpdated), before, threadChannel).ContinueWith(t => HandleInvokeException(t, nameof(ThreadUpdated)));
                                }
                                break;
                            case "THREAD_DELETE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (THREAD_DELETE)");
                                    ...
                                    _ = TimedInvokeAsync(_threadDeleted, nameof(ThreadDeleted), cacheable).ContinueWith(t => HandleInvokeException(t, nameof(ThreadDeleted)));
                                }
                                break;
                            case "THREAD_LIST_SYNC":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (THREAD_LIST_SYNC)");
                            ...
                            case "THREAD_MEMBER_UPDATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (THREAD_MEMBER_UPDATE)");
                            ...
                            case "THREAD_MEMBERS_UPDATE":
                                {
                                    await _gatewayLogger.DebugAsync("Received Dispatch (THREAD_MEMBERS_UPDATE)");
                                    ...
                                    if (leftUsers != null)
                                    {
                                        ...
                                        {
                                            _ = TimedInvokeAsync(_threadMemberLeft, nameof(ThreadMemberLeft), threadUser).ContinueWith(t => HandleInvokeException(t, nameof(ThreadMemberLeft)));
                                        }
                                    }
                                    ...
                                    if (joinUsers != null)
                                    {
                                        ...
                                        {
                                            _ = TimedInvokeAsync(_threadMemberJoined, nameof(ThreadMemberJoined), threadUser).ContinueWith(t => HandleInvokeException(t, nameof(ThreadMemberJoined)));
                                        }
                                    }
                                }
                            ...
                            case "STAGE_INSTANCE_CREATE" or "STAGE_INSTANCE_UPDATE" or "STAGE_INSTANCE_DELETE":
                                {
                                    await _gatewayLogger.DebugAsync($"Received Dispatch ({type})");
                                    ...
                                    switch (type)
                                    {
                                        case "STAGE_INSTANCE_CREATE":
                                            _ = TimedInvokeAsync(_stageStarted, nameof(StageStarted), stageChannel).ContinueWith(t => HandleInvokeException(t, nameof(StageStarted)));
                                            return;
                                        case "STAGE_INSTANCE_DELETE":
                                            _ = TimedInvokeAsync(_stageEnded, nameof(StageEnded), stageChannel).ContinueWith(t => HandleInvokeException(t, nameof(StageEnded)));
                                            return;
                                        case "STAGE_INSTANCE_UPDATE":
                                            _ = TimedInvokeAsync(_stageUpdated, nameof(StageUpdated), before, stageChannel).ContinueWith(t => HandleInvokeException(t, nameof(StageUpdated)));
                                            return;
                                    }
                                }
                                break;
                            ...
                            case "GUILD_SCHEDULED_EVENT_CREATE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_guildScheduledEventCreated, nameof(GuildScheduledEventCreated), newEvent).ContinueWith(t => HandleInvokeException(t, nameof(GuildScheduledEventCreated)));
                                }
                                break;
                            case "GUILD_SCHEDULED_EVENT_UPDATE":
                                {
                                    ...
                                    {
                                        _ = TimedInvokeAsync(_guildScheduledEventCompleted, nameof(GuildScheduledEventCompleted), after).ContinueWith(t => HandleInvokeException(t, nameof(GuildScheduledEventCompleted)));
                                    }
                                    else if ((before != null ? before.Status != GuildScheduledEventStatus.Active : false) && data.Status == GuildScheduledEventStatus.Active)
                                    {
                                        _ = TimedInvokeAsync(_guildScheduledEventStarted, nameof(GuildScheduledEventStarted), after).ContinueWith(t => HandleInvokeException(t, nameof(GuildScheduledEventStarted)));
                                    }
                                    else
                                        _ = TimedInvokeAsync(_guildScheduledEventUpdated, nameof(GuildScheduledEventUpdated), beforeCacheable, after).ContinueWith(t => HandleInvokeException(t, nameof(GuildScheduledEventUpdated)));
                                }
                                break;
                            case "GUILD_SCHEDULED_EVENT_DELETE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_guildScheduledEventCancelled, nameof(GuildScheduledEventCancelled), guildEvent).ContinueWith(t => HandleInvokeException(t, nameof(GuildScheduledEventCancelled)));
                                }
                                break;
                            case "GUILD_SCHEDULED_EVENT_USER_ADD" or "GUILD_SCHEDULED_EVENT_USER_REMOVE":
                                {
                                    ...
                                    if (guildEvent == null)
                                    {
                                        await UnknownGuildEventAsync(type, data.EventId, data.GuildId);
                                        return;
                                    }
                                    ...
                                    switch (type)
                                    {
                                        case "GUILD_SCHEDULED_EVENT_USER_ADD":
                                            _ = TimedInvokeAsync(_guildScheduledEventUserAdd, nameof(GuildScheduledEventUserAdd), cacheableUser, guildEvent).ContinueWith(t => HandleInvokeException(t, nameof(GuildScheduledEventUserAdd)));
                                            break;
                                        case "GUILD_SCHEDULED_EVENT_USER_REMOVE":
                                            _ = TimedInvokeAsync(_guildScheduledEventUserRemove, nameof(GuildScheduledEventUserRemove), cacheableUser, guildEvent).ContinueWith(t => HandleInvokeException(t, nameof(GuildScheduledEventUserRemove)));
                                            break;
                                    }
                                }
                                break;
                            ...
                            case "WEBHOOKS_UPDATE":
                                {
                                    ...
                                    await _gatewayLogger.DebugAsync("Received Dispatch (WEBHOOKS_UPDATE)");

                                    var guild = State.GetGuild(data.GuildId);
                                    var channel = State.GetChannel(data.ChannelId);

                                    _ = TimedInvokeAsync(_webhooksUpdated, nameof(WebhooksUpdated), guild, channel).ContinueWith(t => HandleInvokeException(t, nameof(WebhooksUpdated)));
                                }
                                break;
                            ...
                            case "AUTO_MODERATION_RULE_CREATE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_autoModRuleCreated, nameof(AutoModRuleCreated), rule).ContinueWith(t => HandleInvokeException(t, nameof(AutoModRuleCreated)));
                                }
                                break;
                            ...
                            case "AUTO_MODERATION_RULE_UPDATE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_autoModRuleUpdated, nameof(AutoModRuleUpdated), cacheableBefore, guild.AddOrUpdateAutoModRule(data)).ContinueWith(t => HandleInvokeException(t, nameof(AutoModRuleUpdated)));
                                }
                                break;
                            ...
                            case "AUTO_MODERATION_RULE_DELETE":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_autoModRuleDeleted, nameof(AutoModRuleDeleted), rule).ContinueWith(t => HandleInvokeException(t, nameof(AutoModRuleDeleted)));
                                }
                                break;
                            ...
                            case "AUTO_MODERATION_ACTION_EXECUTION":
                                {
                                    ...
                                    _ = TimedInvokeAsync(_autoModActionExecuted, nameof(AutoModActionExecuted), guild, action, eventData).ContinueWith(t => HandleInvokeException(t, nameof(AutoModActionExecuted)));
                                }
                                break;
                            ...
                            case "GUILD_INTEGRATIONS_UPDATE":
                                ...
                                break;
                            case "MESSAGE_ACK":
                                await _gatewayLogger.DebugAsync("Ignored Dispatch (MESSAGE_ACK)");
                                break;
                            ...
                            default:
                                if (!SuppressUnknownDispatchWarnings)
                                    await _gatewayLogger.WarningAsync($"Unknown Dispatch ({type})");
                                break;
                                #endregion
                        }
                        break;
                    default:
                        await _gatewayLogger.WarningAsync($"Unknown OpCode ({opCode})");
                        break;
                }
            }
            catch (Exception ex)
        ...
        private async Task RunHeartbeatAsync(int intervalMillis, CancellationToken cancelToken)
        {
            try
            {
                await _gatewayLogger.DebugAsync("Heartbeat Started");
                while (!cancelToken.IsCancellationRequested)
                {
                    ...
                    {
                        await _gatewayLogger.WarningAsync("Heartbeat Errored", ex);
                    }

                    await Task.Delay(intervalMillis, cancelToken);
                }
                await _gatewayLogger.DebugAsync("Heartbeat Stopped");
            }
            ...
            {
                await _gatewayLogger.ErrorAsync("Heartbeat Errored", ex);
            }
        }
        ...
        private async Task WaitForGuildsAsync(CancellationToken cancelToken, Logger logger)
        {
            ...
            try
            {
                await logger.DebugAsync("GuildDownloader Started");
                ...
                await logger.DebugAsync("GuildDownloader Stopped");
            }
            catch (OperationCanceledException)
            {
                await logger.DebugAsync("GuildDownloader Stopped");
            ...
            {
                await logger.ErrorAsync("GuildDownloader Errored", ex);
            }
        }
        private async Task SyncGuildsAsync()
        {
            ...
            if (guildIds.Length > 0)
                await ApiClient.SendGuildSyncAsync(guildIds);
        }
        ...
        private Task GuildAvailableAsync(SocketGuild guild)
        {
            if (!guild.IsConnected)
            {
                ...
                return TimedInvokeAsync(_guildAvailableEvent, nameof(GuildAvailable), guild).ContinueWith(t => HandleInvokeException(t));
            }
            return Task.CompletedTask;
        }
        private Task GuildUnavailableAsync(SocketGuild guild)
        {
            if (guild.IsConnected)
            {
                ...
                return TimedInvokeAsync(_guildUnavailableEvent, nameof(GuildUnavailable), guild).ContinueWith(t => HandleInvokeException(t));
            }
            return Task.CompletedTask;
        }
        ...
        private Task UnknownGlobalUserAsync(string evnt, ulong userId)
        {
            ...
            _gatewayLogger.WarningAsync($"Unknown User ({details}).");
            return Task.CompletedTask;
        }
        ...
        {
            string details = $"{evnt} User={userId} Guild={guildId}";
            _gatewayLogger.DebugAsync($"User has not been downloaded ({details}).");
            return Task.CompletedTask;
        }
        private Task UnknownChannelAsync(string evnt, ulong channelId)
        {
            string details = $"{evnt} Channel={channelId}";
            _gatewayLogger.WarningAsync($"Unknown Channel ({details}).");
            return Task.CompletedTask;
        }
        private Task UnknownChannelAsync(string evnt, ulong channelId, ulong guildId)
        {
            if (guildId == 0)
            {
                UnknownChannelAsync(evnt, channelId);
                return Task.CompletedTask;
            }
        ...
        private Task UnknownRoleAsync(string evnt, ulong roleId, ulong guildId)
        {
            ...
            _gatewayLogger.WarningAsync($"Unknown Role ({details}).");
            return Task.CompletedTask;
        }
        private Task UnknownGuildAsync(string evnt, ulong guildId)
        {
            string details = $"{evnt} Guild={guildId}";
            _gatewayLogger.WarningAsync($"Unknown Guild ({details}).");
            return Task.CompletedTask;
        }
        ...
        private Task UnknownGuildEventAsync(string evnt, ulong eventId, ulong guildId)
        {
            ...
            _gatewayLogger.WarningAsync($"Unknown Guild Event ({details}).");
            return Task.CompletedTask;
        }
        private Task UnsyncedGuildAsync(string evnt, ulong guildId)
        {
            string details = $"{evnt} Guild={guildId}";
            _gatewayLogger.DebugAsync($"Unsynced Guild ({details}).");
            return Task.CompletedTask;
        }
        ...
        async Task<IReadOnlyCollection<IVoiceRegion>> IDiscordClient.GetVoiceRegionsAsync(RequestOptions options)
            => await GetVoiceRegionsAsync(options);
        /// <inheritdoc />
        async Task<IVoiceRegion> IDiscordClient.GetVoiceRegionAsync(string id, RequestOptions options)
            => await GetVoiceRegionAsync(id, options);
